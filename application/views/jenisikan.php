...
?>
                ...
                <div class="content">
                    <div class="container">
                        <div class="row">
							<div class="col-xs-12">
								<div class="page-title-box">
                                    <h4 class="page-title">Jenis Ikan </h4>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card-box">
                                  <button onClick="tambahIkan()" type="button" class="btn btn-sm btn-success waves-effect w-md waves-light m-b-5">
        ...
        <script type="text/javascript">
            ...
            save = () => {
                if(save_method == 'add'){
                    url = 'JenisIkan/tambahIkan'
                }else{
                    url = 'JenisIkan/updateIkan'
                }
                $.ajax({
                    url: url,
                    type: "POST",
            ...
            tambahIkan = () => {
               save_method = 'add';
               $('.modal-title').text('Tambah Ikan');
               $('form')[0].reset();
               
               $('#modalJenisIkan').modal('show');         
               $('input[name="namaikan"]').focus();
            }
            editIkan = (id) => {
                save_method = 'edit';
                $('form')[0].reset();
                $.ajax({
                    url : `JenisIkan/editIkan/${id}`,
                    type: "GET",
                    dataType: "JSON",
                    success: function(data){
                        $('[name="idikan"]').val(data.IDIkan);
                        $('[name="namaikan"]').val(data.NamaIkan);
                        $('#modalJenisIkan').modal('show');
                    }
                })
            }
            ...
            $(document).ready(function(){
                table = $('#mytable').DataTable({
                    ...
                    "order" : [],
                    "ajax" :{
                        "url" : "<?php echo site_url('JenisIkan/ajax_list');?>",
                        "type" : "POST"
                    },
                    "columnDefs": [{
                        "targets": [ -1 ],
